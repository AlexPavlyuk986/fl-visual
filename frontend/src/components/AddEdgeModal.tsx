...
function AddEdgeModal({
  ...
  const canAdd = source.trim() !== "" && target.trim() !== "";
  ...
  const handleAdd = async () => {
    ...
    if (!sourceExists) {
      setWarning(`Node "${source}" does not exist.`);
    ...
    if (!targetExists) {
      setWarning(`Node "${target}" does not exist.`);

      return;
    }

    if (source === target) {
      setWarning("Source and target must be different.");
    ...
    if (!success) {
      setWarning("Edge already exists.");
